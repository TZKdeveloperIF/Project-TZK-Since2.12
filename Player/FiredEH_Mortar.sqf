...
if (local _shell) then {
	if (player == gunner _vehicle) then {
		_scale = 0;
		if (_weapon == "Mortar_500m_xj300") then {_scale = 80};
		if (_weapon == "Mortar_1000m_xj300") then {_scale = 120};
		if (_weapon == "Mortar_1500m_xj300") then {_scale = 140};
	...
	[_vehicle, _shell, _limitWeaponRangeFactor] exec "Player\LimitWeaponRangeNew.sqs";
