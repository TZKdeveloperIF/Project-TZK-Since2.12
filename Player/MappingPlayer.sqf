...
"_i", "_v", "_m", "_radars", "_siEnemy", "_SpySatPos"];
...
if (count _radars > 0 || bSpySat) then
{
	_SpySatPos = [SpySatPos0, SpySatPos1] select siPlayer;
	...
	while "_i < maxVehicleMarkers" do
	{
		...
		{
			if ( 50 > ((getPos _v) select 2) && !bSpySat ) then {_m setMarkerPos hiddenMarkerPos} else
			{
				_inrange = false;
				if (count _radars > 0) then { { if ((_v distance _x) < 4000) then { _inrange = true } } foreach _radars; };
				if (bSpySat) then {if ( ([getPos _v, _SpySatPos] call funcDistH) < (2*viewDistance) ) then { _inrange = true }; };
				if ( !_inrange ) then {_m setMarkerPos hiddenMarkerPos;} else
				{
					_m setMarkerPos getPos _v;
				};
			};
