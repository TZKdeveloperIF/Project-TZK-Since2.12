private ["_vx", "_vy", "_yS", "_yE"];
private ["_c", "_g", "_h", "_t", "_x", "_y", "_maxH"]; 
private ["_vx0", "_vy0",  "_K1", "_L1", "_K2", "_L2", "_K3", "_L3", "_K4", "_L4"];

_vx = _this select 0; _vy = _this select 1; _yS = _this select 2; _yE = _this select 3;

_x = 0;	_y = _yS;	_maxH = _yS;
...
while "_y >= _yE" do {
	...
	_x=_x+_h/2*(_vx0+_vx);
	_y=_y+_h/2*(_vy0+_vy);
	if(_vy0 >= 0 && _vy <= 0) then "_maxH = _y";
};
_x = (if (_vy0 * _vy > 0) Then {_t = (_y-_yE)/(_vy0+_vy), _x - _t*(_vx0+_vx)} Else {_x});
[_x, _maxH]