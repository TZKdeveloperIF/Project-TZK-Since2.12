...
_bool_NoCheckX = if (count _this > 3) Then {_this select 3} Else {true};
_c = if (count _this > 4) Then {_this select 4} Else {-0.0005};

_x = 0; _y = 0;
_g = 9.80665; _h = 0.05;

private ["_condition", "_return"];
if _bool_NoCheckX Then {
	_condition = "_x < _deltaX", _return = "_y - (_vy0+_vy) * (_x-_deltaX)/(_vx0+_vx)"
} Else {
	_condition = "_x < _deltaX && _y >= 0", _return = "if (_y < 0) Then {-1} Else {_y - (_vy0+_vy) * (_x-_deltaX)/(_vx0+_vx)}"
};
