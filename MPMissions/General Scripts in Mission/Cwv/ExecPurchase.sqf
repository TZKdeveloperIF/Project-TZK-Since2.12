...
_k = 0; _c = count _shoppingCartMag; while {_k < _c} do {
	...
	[_magInfo select _infoIdx select _piName, _si, _enum, _vehicle] call preprocessFile "Util\CWV\ProperMagWep.sqf";
...
_m = []; _w = [];
_vehicle call preprocessFile "Cwv\AppendGeneralEquips.sqf";
{_vehicle addMagazine _x} forEach _m; {_vehicle addWeapon _x} forEach _w;

_vehicle selectWeapon (weapons _vehicle select 0);

[_vehicle, (_info select 0) + _upgradeCost, true, _enum] exec "Cwv\Net\sCwvInfo.sqs";