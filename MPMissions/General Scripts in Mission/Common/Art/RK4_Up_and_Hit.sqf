...
_vx = _this select 0; _vy = _this select 1; _yS = _this select 2; _xE = _this select 3; _retType = (if(count _this > 4) then "_this select 4" else "0");
...
_t = (_x - _xE)/(_vx0+_vx); _y - (_vy0+_vy)*_t