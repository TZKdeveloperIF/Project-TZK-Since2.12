...
_height = [_vx, _vy, _distX, false] call loadFile "Common\SQF\RK4_X_Y.sqf";
if (_height == -1) Then {_i = 0, _try = 3, while "_i < _try && _theta0 < 83 && _height == -1" do {
	...
	_vx = _speed * cos(_theta0); _vy = _speed * sin(_theta0);
	_height = [_vx, _vy, _distX, false] call loadFile "Common\SQF\RK4_X_Y.sqf"; _i = _i + 1
...
if (_height == -1) Then {_vehicle groupChat "Failed to shoot long. Shell deleted.", deleteVehicle _shell, exit} Else {
	if (_speedMax < _speed) Then {call loadFile "Common\EH\Fired_Revise_Angle_Case_11_001.sqf"} Else {
		_theta1 = _theta0 + 0.5; _vx = _speed * cos(_theta1); _vy = _speed * sin(_theta1);
		_height = [_vx, _vy, _distX] call loadFile "Common\SQF\RK4_X_Y.sqf"; _error1 = _height - _deltaY;
		...
		_error = 100; while "abs(_error) > 8 && _i < _try" do {
			_vx = _speed * cos(_theta); _vy = _speed * sin(_theta); _height = [_vx, _vy, _distX] call loadFile "Common\SQF\RK4_X_Y.sqf"; _error = _height - _deltaY;
