...
if (not _checked && -1 == _vt) then {
	_checked = true;
	_rearm = not (_veh call preprocessFile "Util\NeedntRearmGeneral.sqf");
};
// Return true for artillery
if (not _checked && (_vt in typesHowitzer || _vt in typesRocketLauncher)) then {
	_checked = true;
	_rearm = true;
};
