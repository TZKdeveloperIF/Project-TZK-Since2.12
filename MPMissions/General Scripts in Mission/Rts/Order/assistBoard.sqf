...
_i = 0; _c = count _nearByTargets; while {_i < _c} do {
	...
	if (_vt in _selectedTypes && not isNull driver _object && canMove _object && (getPos _object) select 2 < 5 && speed _object < 1) then {
		_slot = (_object call funcGetTranspVol) - (_object call funcGetCargoNum);
		if (_slot > 0) then {
