...
if (not _processed && 0 == count _downBoardStack) then {
	...
	if (isCommander || bIsAiSuperior) then {
		ctrlSetText [_idcDownBtn0, format ["%1 %2", localize {TZK_LANG_AI}, localize {TZK_LANG_SHORT_ARTILLERY}]];
	...
	// mine area
	if (isCommander || bIsAiSuperior) then {
		ctrlSetText [_idcDownBtn0 + 3, format ["%1 %2", localize {TZK_LANG_AI}, localize {TZK_LANG_MINE_VERB}]];
