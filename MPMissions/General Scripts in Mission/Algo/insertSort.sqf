...
if (_begin >= _end) then {
	...
	_i = _begin + 1; while {_i < _end} do {
		...
		_j = [_array, _begin, _i, _elem, _this select 3, _bPreprocess] call preprocessFile "Algo\lowerBound.sqf";
		[_array, _i, _j, _elem] call preprocessFile "Algo\vectorInsert.sqf";
		_i = _i + 1;
	};
}