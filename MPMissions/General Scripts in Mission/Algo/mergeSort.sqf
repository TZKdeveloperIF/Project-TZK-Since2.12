...
if (_begin >= _end || _begin < 0 || _end > count _array) then {
	...
	if (_end - _begin <= 16) then {
		[_array, _begin, _end, _this select 3, _bPreprocess] call preprocessFile "Algo\insertSort.sqf";
	} else {
		_mid = (_begin + _end) / 2;
		_mid = _mid - (_mid % 1) + 1;

		[_array, _begin, _mid, _this select 3, _bPreprocess] call preprocessFile "Algo\mergeSort.sqf";
		[_array, _mid, _end, _this select 3, _bPreprocess] call preprocessFile "Algo\mergeSort.sqf";

		[_array, _begin, _mid, _end, _this select 3, _bPreprocess] call preprocessFile "Algo\inplaceMerge.sqf";
	};
};