...
if !bool_TZK_199_Mode then {
	_bVerify = true;
	if (_bVerify && "A10_xj400" countType [_vehicle] > 0) then {_totalSlots = 8.99; _bVerify = false};
	if (_bVerify && "Su25_xj400" countType [_vehicle] > 0) then {_totalSlots = 6.99; _bVerify = false};
	if (_bVerify && _enum == enum_TZK_CustomVehicle_Gunship) then {
		_totalSlots = 8.99,  if ("Ka50_xj400" countType [_vehicle] > 0) then {_totalSlots = 12.99}; _bVerify = false;
	};
};