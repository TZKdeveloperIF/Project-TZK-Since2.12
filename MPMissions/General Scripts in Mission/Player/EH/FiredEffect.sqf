...
_skipTank = UsedVersion >= 2020; _skipMg = UsedVersion >= 2020;
_bShell = false; _sizeOfShell = 0;
	if not _bShell then {if (-1 != ["M252Launcher", "Mortar81_Launcher_xj400", "Mortar82_Launcher_xj400"] find _weapon) then {
		_bShell = true; _sizeOfShell = 81;
	}};
	if not _bShell then {if (-1 != ["Gun120_Grk_xj400", "Gun122_VME_xj400"] find _weapon) then {_bShell = true; _sizeOfShell = 122}};
	if not _bShell then {if (-1 != ["Gun155_xj400","PLA155Gun_xj400", "Gun155"] find _weapon) then {_bShell = true; _sizeOfShell = 155}};

	if not _skipTank then {
		if not _bShell then {if (-1 != ["Gun105_xj400", "Gun106_htr_xj400", "105RC_OFrP_xj400"] find _weapon) then {_bShell = true; _sizeOfShell = 105}};
		if not _bShell then {if (-1 != ["Gun122_xj400"] find _weapon) then {_bShell = true; _sizeOfShell = 122}};
		if not _bShell then {if (-1 != ["Gun130_xj400"] find _weapon) then {_bShell = true; _sizeOfShell = 130}};
		if not bool_TZK_SEMod_Mode then {if (not _bShell) then {
			if not _bShell then {if (-1 != [
			...
			if not _bShell then {if (-1 != ["Gun73", "MFCTI116BMP1Gun", "Gun_Spg_fdf_xj400"] find _weapon) then {_bShell = true; _sizeOfShell = 73}};
			if not _bShell then {if (-1 != ["HVG90_BOH_xj400", "Zwige_BOH_xj400"] find _weapon) then {_bShell = true; _sizeOfShell = 90}};
			if not _bShell then {if (-1 != ["Gun105", "Gun105_xj400", "MFCTI116M60Gun","MFCTI116M60A3Gun","MFCTI116M1Gun"] find _weapon) then {
				_bShell = true; _sizeOfShell = 105
			}};
			if not _bShell then {if (-1 != ["M1Gun_xj400","M12Gun_xj400","LeoGun_xj400","Gun120", "MFCTI116M1A1Gun"] find _weapon) then {
				_bShell = true; _sizeOfShell = 120
			}};
		}};
	};
...
if (not isNull _effectShell) then {
	...
	if not _found then {if (boole_Local_Cannon_Tracer && _local || boole_Global_Cannon_Tracer && not _local) then {
		...
		] find _weapon) then {[_effectShell, _weapon] exec (TzkScripts select 033)};
	}};
	if (_bShell) then {
		if ((boole_Local_Shell_Tracer || _sizeOfShell > 125) && _local) then {[_local, _shell, _sizeOfShell] exec (TzkScripts select 032)};
		if ((boole_Global_Shell_Tracer || _sizeOfShell > 125) && not _local) then {[_local, _effectShell, _sizeOfShell, _position, _velocity, _dir, _vehicle, _shell] exec (TzkScripts select 032)};
	};
};