...
private [{_mag},{_speed},{_magArr}, {_found},{_i},{_c}, {_name},{_len}];

_mag = ""; _speed = _this select 1;
_magArr = magazinesArray (_this select 0); _found = false;
_i = 0; _c = count _magArr; while {_i < _c && not _found} do {
	...
	if (_len == 21) then { // Shrapnel_W_0080_xj400, Shrapnel_E_0080_xj400
		if (substr [_name, 0, 8] == "Shrapnel" && _speed <= 700) then {_found = true, _mag = _name};
	};
