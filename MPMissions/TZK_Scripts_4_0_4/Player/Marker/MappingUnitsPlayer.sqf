private ["_index", "_count", "_leader", "_markerType", "_marker", "_x",  "_ids", "_id", "_units",  "_gi", "_countGroups", "_group", "_marker", "_ai",  "_i", "_v", "_m"];
...
while "_gi < _countGroups" do {
	...
	while "_index < 12" do {
		...
		if ( _gi == giPlayer && bNumberedAI ) then {_marker setMarkerPos hiddenMarkerPos} else {
