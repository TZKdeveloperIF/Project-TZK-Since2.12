_range = [_vx, _vy, _deltaY] call loadFile "\TZK_Scripts_4_0_4\Common\SQF\RK4_Range_Y_0.sqf"; _error0 = _range - _distX;
if (_error0 < -0.5 * _distX) Then {_vehicle groupChat "Improper initial elevation angle. Shell deleted.", deleteVehicle _shell, exit} Else {
	...
	_range = [_vx, _vy, _deltaY] call loadFile "\TZK_Scripts_4_0_4\Common\SQF\RK4_Range_Y_0.sqf"; _error1 = _range - _distX;
	...
	_error = 100; while "abs(_error) > 8 && _i < _try" do {
		_vx = _speed * cos(_theta); _vy = _speed * sin(_theta); _range = [_vx, _vy, _deltaY] call loadFile "\TZK_Scripts_4_0_4\Common\SQF\RK4_Range_Y_0.sqf"; _error = _range - _distX;
