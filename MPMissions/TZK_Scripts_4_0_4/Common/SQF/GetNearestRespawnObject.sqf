...
if (alive _mhq) then {_obj = _mhq; _distMin = _objOld distance _mhq};

_MCVs = MCV select _si;
if ("alive _x " count _MCVs > 0) then {
	{
		if ( (_objOld distance _x) < _distMin && alive _x ) then {_obj = _x; _distMin = _objOld distance _x};
	} foreach _MCVs;
};

{
	...
	{
		if ( (_objOld distance _x) < _distMin ) then {_obj = _x; _distMin = _objOld distance _x}
	} foreach _structs
