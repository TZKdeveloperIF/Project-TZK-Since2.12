...
comment {The "find" or "in" command is cAsE-SeNsItIvE, thus better use "==" to compare string} ;
if (!_found) then {
	_array = ["afganeveron", "Arabia", "asahmi", "noe_desert", "Bahadur", "iraq", 
...
if (!_found) then {
	comment {TheWorld here must be "C" now. Some islands here is defined that never allow "G" version} ;
	comment {If the island playing isn't included in, it'll have 50% to be "G" version} ;
	_array = [];
	_i = 0; _c = count _array; while {_i < _c && !_found} do {
		if (_world == _array select _i) then {_found = true}; _i = _i + 1;
	};
...
comment {Ask server to roll the world} ;
if (isServer && !bool_TZK_199_Mode) then {
	if ("Undefined" == TheWorld) then {
		comment {delay. Make sure server broadcast string after game has started} ;
		[
		...
		if (TheWorld != "C" && TheWorld != "G") then {
			comment {[] exec "\TZK_Scripts_4_0_4\Common\Init\Secondary.sqs"} ;
		}
	};
};