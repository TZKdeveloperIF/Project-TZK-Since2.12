comment {args: update "current selection" in parameter lists: bool};

comment {arg must be bool otherwise endless loop will appear};
if (_selOrder != -1) then {if (_this || not _this) then {
	...
	_i = 0; _c = count _paramDefs; while {_i < _c} do {
		...
		comment {update "current selection" in parameter lists when required};
