comment {args: typeInfos};
comment {read other external variables directly};
private [{_typeInfos}];
_typeInfos = _this;
...
private [{_type},{_num},{_driver},{_gunner}, {_factoryType}];
{
	...
	if (count _assignedFacs <= 0) then {
		comment {Buy at old fashion};
		comment {todo: To improve concurrency, optimize the concept of "idle factory / buildingsInUse" would be a good choice};
		_factories resize 0;
		...
		comment {Buy at assigned factories};
