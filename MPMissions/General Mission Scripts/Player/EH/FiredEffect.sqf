...
_booleShell = false; _sizeOfShell = 0;
	if (_weapon in ["M252Launcher"]) then {_booleShell = true; _sizeOfShell = 81};	// Mortar Structure.
	if (_weapon in ["Gun105_xj400"]) then {_booleShell = true; _sizeOfShell = 105};	// M119 Howitzer.
	if (_weapon in ["Gun122_xj400"]) then {_booleShell = true; _sizeOfShell = 122};	// D30A Howitzer.
	if (_weapon in ["Gun130_xj400"]) then {_booleShell = true; _sizeOfShell = 130};	// M46 Field Gun. Abandoned.
	if (_weapon in ["Gun155_xj400","PLA155Gun_xj400"]) then {_booleShell = true; _sizeOfShell = 155};	// M109/PLZ05 Howitzer.
if (!bool_TZK_SEMod_Mode && !_booleShell) Then {
...
if (!isNull _effectShell) then {
	...
	if ( _type in ["CoaxW_xj400","CoaxE_xj400","50calW_xj400","50calE_xj400","Bullet7_6","Bullet12_7", "MFCTI116Bullet12_7","MFCTI116Bullet7_6"] ) then {
		if ((boole_Local_Bullet_Tracer && _local) || (boole_Global_Bullet_Tracer && !_local)) then {[_effectShell] exec {Player\Effect\Fired_SLX_MG.sqs}};
	};
	...
	if ( _weapon in ["MachineGun30","MachineGun30W","M197_xj400","VulcanCannon","VulcanCannon_xj400","Cannon_20HE_xj400","Cannon_20AP_xj400", "RMKHETiger_DVD_xj400","RMKAPTiger_DVD_xj400", "MachineGun30E","ZsuCannon","Cannon30_Kamov_xj400","CannonE_20HE_xj400","CannonE_20AP_xj400",  "MachineGun30A10","MachineGun30A10Burst","RKTHunter_Guns_xj400","Cannon_Heli_xj400","Cannon_30APHE_xj400"] ) then {
		if ((boole_Local_Cannon_Tracer && _local) || (boole_Global_Cannon_Tracer && !_local)) then {[_effectShell, _weapon] exec {Player\Effect\Fired_FFUR_Cannon.sqs}};
	};
	...
	// Always tracer size > 125 shell.
		if ((boole_Local_Shell_Tracer || _sizeOfShell > 125) && _local) then {[_local, _shell, _sizeOfShell] exec {Player\Effect\Fired_COC_Tracer.sqs}};
		if ((boole_Global_Shell_Tracer || _sizeOfShell > 125) && !_local) then {[_local, _effectShell, _sizeOfShell, _position, _velocity, _dir, _vehicle, _shell] exec {Player\Effect\Fired_COC_Tracer.sqs}};
	};
};
