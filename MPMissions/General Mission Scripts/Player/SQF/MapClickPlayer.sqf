...
// test
//	if (false && (count _units) > 1 && !_processed && !_alt && !_shift) then {_processed = true};

// ai order dialog
	if (!_processed && !_alt && _shift) then {[_pos, _units] exec "Player\Dialog\OrderPlayerAI.sqs"; _processed = true};
	if (!_processed && _alt && _shift) then {[_pos, _units, 2] exec "Player\Dialog\OrderPlayerAI.sqs"; _processed = true};
};

if (!_processed && (count _units) == 0) then {
// test
//	if (false && !_processed && !_alt && !_shift) then {player globalchat format["InForest = %1", [_pos] call funcInForest]; _processed = true};

// unit cam
	if (!_processed && !_alt && _shift && count ([siPlayer, stSatRec] call funcGetWorkingStructures) > 0) then {
		_res = [siPlayer, _pos] call funcGetClosestUnit; _marker = if !bool_TZK_199_Mode Then {(_res select 0) call loadFile "Common\SQF\GetUnitMarker.sqf"} Else {""};
...
// set player waypoint
	if (!_processed && _alt && !_shift) then {[_pos] exec "Player\SetWaypointSmart.sqs"; _processed = true};
